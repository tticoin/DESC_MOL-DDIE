...
from modeling_pretraining import BertForSequenceClassification

#from transformers import glue_compute_metrics as compute_metrics
#from metrics_ddie import ddie_compute_metrics as compute_metrics
from metrics_ddie import pretraining_compute_metrics as compute_metrics
...
def train(args, train_dataset, model, tokenizer, storage_model):
    ...
    fingerprint = np.load(os.path.join(args.fingerprint_dir, 'database_train.npy'), allow_pickle=True)
    ...
    for epoch, _ in enumerate(train_iterator, 1):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and not args.tpu:
                ...
                model.zero_grad()
                global_step += 1
                if args.parameter_averaging: storage_model.accumulate_params(model)
        ...
        # Save model
        if args.use_desc:
            torch.save(model.desc_bert.state_dict(), os.path.join(args.output_dir, 'epoch{}'.format(epoch), 'desc_bert_state_dict'))
            torch.save(model.desc_conv.state_dict(), os.path.join(args.output_dir, 'epoch{}'.format(epoch), 'desc_conv_state_dict'))
        if args.use_mol:
            torch.save(model.gnn.state_dict(), os.path.join(args.output_dir, 'epoch{}'.format(epoch), 'gnn_state_dict'))
        torch.save(model.state_dict(), os.path.join(args.output_dir, 'epoch{}'.format(epoch), 'model_state_dict'))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        fingerprint = np.load(os.path.join(args.fingerprint_dir, 'database_dev.npy'), allow_pickle=True)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        label_list = processor.get_labels(mode='pretraining')
    ...
    for drug_indx in (1,2):
        ...
        if os.path.exists(cached_desc_features_file) and not args.overwrite_cache:
            ...
            label_list = desc_processor.get_labels(mode='pretraining')
            if task in ['mnli', 'mnli-mm'] and args.model_type in ['roberta']:
                # HACK(label indices are swapped in RoBERTa pretrained model)
                label_list[1], label_list[2] = label_list[2], label_list[1] 
...
def main():
    ...
    if args.pretrained_dir is not None and not args.do_train:
        model.load_state_dict(torch.load(os.path.join(args.pretrained_dir, 'model_state_dict')))
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and not args.tpu:
        ...
        if args.use_desc:
            torch.save(model.desc_bert.state_dict(), os.path.join(args.output_dir, 'desc_bert_state_dict'))
            torch.save(model.desc_conv.state_dict(), os.path.join(args.output_dir, 'desc_conv_state_dict'))
            if args.desc_layer_hidden != 0:  torch.save(model.W_desc.state_dict(), os.path.join(args.output_dir, 'desc_W_state_dict'))
        if args.use_mol:
            torch.save(model.gnn.state_dict(), os.path.join(args.output_dir, 'gnn_state_dict'))
        '''Do not save model
        logger.info("Saving model checkpoint to %s", args.output_dir)
